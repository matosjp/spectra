\documentclass[preprints,article,submit,moreauthors,pdftex]{Definitions/mdpi}
...
\Title{Stellar Parameter Determination in Open Clusters: A Comparative Benchmark of 2D Bayesian Isochrone Fitting, Machine Learning Regressors, and Empirical Calibrations}

\Author{João Paulo Matos Dias Gomes $^{1,2,\ddagger}$\ast, Maria Jaqueline Vasconcelos $^{2}$ and Adriano Hoth Cerqueira $^{2}$}

\AuthorNames{João Paulo Matos Dias Gomes, Maria Jaqueline Vasconcelos and Adriano Hoth Cerqueira}

\address{%
$^{1}$ \quad Department of Physics, Institute of Biosciences, Humanities and Exact Sciences, São Paulo State University (UNESP), São José do Rio Preto, SP 15054-000, Brazil\\
$^{2}$ \quad Laboratório de Astrofísica Teórica e Observacional (LATO), Universidade Estadual de Santa Cruz (UESC), Ilhéus, BA 45662-900, Brazil}

\corres{Correspondence: jpmdgomes.bf@gmail.com}

\abstract{
Determining fundamental stellar parameters --- such as mass ($M_\odot$), age ($t$), effective temperature ($T_{\text{eff}}$), surface gravity ($\log g$), bolometric luminosity ($\log_{10}(L/L_\odot)$), and stellar radius ($R_*$) --- is essential for constraining star formation histories, IMF variations, and disk accretion physics across open clusters. In this work, we present an empirical benchmark comparing 2D Bayesian Isochrone Fitting (\texttt{IsocFit}) incorporating Salpeter/Kroupa IMF priors, semi-empirical Machine Learning Mass--Magnitude Relationships (RMM), and spectroscopic Pre-Main-Sequence (PMS) activity classifications across five open clusters spanning evolutionary stages from $\sim 1$ to $\sim 150$~Myr: Orion Nebula Cluster (ONC, $\sim 1$~Myr), Upper Scorpius ($\sim 5\text{--}11$~Myr), $h$~Persei / NGC~869 ($\sim 13$~Myr), Pleiades ($\sim 112$~Myr), and NGC~2516 ($\sim 150$~Myr). Methodological validation against eclipsing binary control stars demonstrates that $k$-NN regressors trained on typical stellar evolutionary tracks reproduce empirical masses with $3.8\%$ median residual uncertainty ($R^2 = 0.96$, homoscedasticity $P=0.12$). For young PMS clusters ($<10$~Myr), 2D Bayesian isochrone likelihood estimation accurately captures age dispersion and individual $1\text{-}\sigma$ posterior uncertainties, whereas $k$-NN ML models using \textit{Gaia} DR3 $G/V$ magnitudes minimize extinction and bolometric correction sensitivity. Conversely, for clusters near or beyond Zero-Age Main Sequence (ZAMS) convergence ($>100$~Myr), $k$-NN regressors resolve track-folding degeneracies, yielding superior agreement with reference catalogues ($R^2 \approx 0.96\text{--}0.99$). Finally, incorporating spectroscopic Equivalent Widths ($H\alpha$, $\text{Li}\,\text{I}$, $\text{TiO}$) provides an robust boundary distinguishing Classical T~Tauri Stars ($H\alpha \ge 10\,\text{\AA}$) from Weak-lined T~Tauri and field stars. All algorithms are implemented in the open-source software \textsc{SPECTRA} (v1.2.0).
}

\keyword{stars: pre-main-sequence; stars: fundamental parameters; open clusters and associations: general; stars: low-mass; Hertzsprung-Russell and colour-magnitude diagrams}

% Astronomy journal macro definitions
...
\begin{equation}
\begin{split}
    L(d | M, t) &= \frac{1}{2\pi \sigma_T \sigma_L} \\
    &\quad \times \exp \Bigg[ -\frac{1}{2} \Bigg( \frac{(T_{\text{eff,obs}} - T_{\text{eff,grid}}(M,t))^2}{\sigma_T^2} \\
    &\quad + \frac{(\log L_{\text{obs}} - \log L_{\text{grid}}(M,t))^2}{\sigma_L^2} \Bigg) \Bigg]
\end{split}
\label{eq:likelihood}
\end{equation}
...
Theoretical evolutionary tracks supported in \texttt{IsocFit} include those from \citet{siess2000internet} and BHAC15 \citep{baraffe2015new}. The choice of these tracks is historically and physically motivated: \citet{siess2000internet} grids remain a heavily referenced standard for PMS evolution spanning a wide mass range, providing a long-standing baseline for comparing legacy studies, while the BHAC15 tracks incorporate updated solar abundances, atmospheric boundary conditions, and improved convection mixing-length calibrations, making them particularly well-suited for very-low-mass stars and brown dwarfs. Combining these two complementary grids ensures comprehensive coverage of both classical and modern evolutionary constraints.
...
For catalogs lacking pre-computed effective temperatures or luminosities, the \texttt{Primary Parameters} module derives fundamental physical quantities directly from multi-band photometry (\textit{Gaia} $G, BP, RP$, 2MASS $J, H, K$, Johnson $V, B$).
...
\begin{enumerate}
    \item \textbf{Distance Modulus ($\text{DM}$):} Calculated from stellar distance $d_{\text{pc}}$ or parallax $\varpi_{\text{mas}}$:
    \begin{equation}
        \text{DM} = 5 \log_{10}(d_{\text{pc}}) - 5 = 15 - 5 \log_{10}(\varpi_{\text{mas}})
        \label{eq:dm}
    \end{equation}
    \item \textbf{Absolute Bolometric Magnitude ($M_{\text{bol}}$):}
    \begin{equation}
        M_\lambda = m_{\lambda,0} - \text{DM}, \quad M_{\text{bol}} = M_\lambda + \text{BC}_\lambda(T_{\text{eff}})
        \label{eq:mbol}
    \end{equation}
    \item \textbf{Logarithmic Bolometric Luminosity ($\log_{10}(L_*/L_\odot)$):}
    \begin{equation}
    ...
    \item \textbf{Stellar Radius ($R_*/R_\odot$):} Via the Stefan--Boltzmann law:
    \begin{equation}
...
\begin{itemize}
    \item \textbf{Classical T Tauri Stars (CTTS):} Strong circumstellar disk accretion emission ($\text{EW}(\text{H}\alpha) \ge 10\,\text{\AA}$ for K--M spectral types).
    \item \textbf{Weak-lined T Tauri Stars (WTTS):} Chromospheric magnetic activity without heavy disk accretion ($2\,\text{\AA} \le \text{EW}(\text{H}\alpha) < 10\,\text{\AA}$) accompanied by lithium retention ($\text{EW}(\text{Li}\,\text{I}) \ge 0.1\,\text{\AA}$).
    \item \textbf{Field Stars:} Absence of significant $\text{H}\alpha$ emission and depleted lithium.
\end{itemize}
...
\begin{figure}[H]
	...
	\caption{Validation of \texttt{IsocFit} 2D Bayesian mass recovery using empirical double-lined eclipsing binary stars from the Pleiades \citep[e.g.,][]{delfosse2000accurate, benedict2016solar}. The dashed diagonal line denotes 1:1 identity.}
	\label{fig:validation_synthetic}
\end{figure}

Fig.~\ref{fig:validation_synthetic} shows the recovery of stellar masses using \texttt{IsocFit} against empirical data from the Pleiades binaries. The strong alignment with the 1:1 identity line validates the robust performance of the 2D Bayesian mass recovery algorithm on observational benchmarks.
...
Upper Scorpius represents a key PMS transition phase ($d \approx 140$~pc). Combining \textit{Gaia} DR3 photometry with TESS V8.2 luminosities \citep{stassun_revised_2019} and \citet{stauffer2019rotational} catalogs, member stars were localized on the HR diagram (Fig.~\ref{fig:usco_hrd}).
...
Fig.~\ref{fig:usco_results} illustrates mass comparisons. For low-mass stars ($M \le 0.35\,M_\odot$), $k$-NN RMM modeling using \textit{Gaia} $G$-band magnitude achieves excellent linear alignment without systematic bias, outperforming 1D isochrone interpolation.
...
The Pleiades cluster ($d \approx 136.2$~pc) provides an ideal benchmark near ZAMS convergence. Using high-precision \textit{Gaia} DR3 astrometry and photometry from \citet{lodieu2019fiveD}, member stars were placed on the HR diagram (Fig.~\ref{fig:ple_hrd}).
...
\begin{enumerate}
    \item \textbf{Young PMS Clusters ($t < 10$~Myr):} 2D Bayesian \texttt{IsocFit} is essential when individual stellar ages and posterior standard deviations ($\sigma_M, \sigma_t$) are required to quantify cluster age spreads. However, high differential extinction ($A_V$) and accretion veiling increase HR position scatter.
    \item \textbf{Main-Sequence Clusters ($t > 100$~Myr):} As stars settle onto the ZAMS, theoretical tracks merge, causing HR grid minimization degeneracies. Here, $k$-NN machine learning regressors trained on single-band magnitudes (\textit{Gaia} $G$ or $V$) eliminate track-folding ambiguity, yielding superior mass recovery ($R^2 \ge 0.96$).
    \item \textbf{Spectroscopic Activity Filtering:} Incorporating Equivalent Widths ($\text{H}\alpha$, $\text{Li}\,\text{I}$) prevents actively accreting CTTS stars ($\text{EW}(\text{H}\alpha) \ge 10\,\text{\AA}$) from skewing photometric $T_{\text{eff}}$ dereddening vectors.
\end{enumerate}
...
\begin{enumerate}
    ...
    \item Machine-learning Mass--Magnitude relationships (RMM) using $k$-NN regressors outperform traditional grid interpolation for stars near the ZAMS ($t \ge 100$~Myr), achieving median mass uncertainties of $2.1\text{--}3.8\%$. This precision strongly contextualizes our results against comprehensive surveys of the solar neighbourhood; for instance, \citet{das2019masses} derive median uncertainties around $8.5\%$ using Bayesian neural networks on \textit{Gaia} and \textit{TGAS} data, affirming the high fidelity of our cluster-specific localized regression models.
...
Overall, the fusion of Bayesian inference with machine learning regressors mitigates theoretical grid degeneracy while retaining formal error propagation. All algorithms, models, and interactive HTML report generators are publicly available in the \textsc{SPECTRA} software package (\url{https://github.com/matosjp/spectra}).


\vspace{6pt} 

\authorcontributions{Conceptualization, J.P.M.D.G., M.J.V. and A.H.C.; methodology, J.P.M.D.G. and A.H.C.; software, J.P.M.D.G.; validation, J.P.M.D.G., M.J.V. and A.H.C.; formal analysis, J.P.M.D.G.; investigation, J.P.M.D.G.; resources, M.J.V.; data curation, J.P.M.D.G.; writing---original draft preparation, J.P.M.D.G.; writing---review and editing, J.P.M.D.G., M.J.V. and A.H.C.; visualization, J.P.M.D.G.; supervision, M.J.V. and A.H.C.; project administration, M.J.V. and A.H.C.; funding acquisition, M.J.V. and A.H.C. All authors have read and agreed to the published version of the manuscript.}

\funding{This research was partially funded by UESC projects 2023.0003119-74 and 2025.0033125-91.}

\institutionalreview{Not applicable.}

\informedconsent{Not applicable.}

\dataavailability{All observational datasets, theoretical model grids, synthetic control tables, and derived cluster parameter catalogs underlying this article are available in the \textsc{SPECTRA} repository at \url{https://github.com/matosjp/spectra/tree/main/paper_data}.}

\acknowledgments{The primary development, algorithmic design, and empirical cluster validations were conducted at the Laboratório de Astrofísica Teórica e Observacional (LATO) at the Universidade Estadual de Santa Cruz (UESC). The authors acknowledge institutional support provided by the Universidade Estadual Paulista (UNESP) during final manuscript preparation.}
...
\bibliography{universe_references}
