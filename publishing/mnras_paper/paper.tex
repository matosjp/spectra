...
\begin{enumerate}
    ...
    \item \textbf{Main-Sequence Clusters ($t > 100$~Myr):} As stars settle onto the ZAMS, theoretical tracks merge, causing HR grid minimization degeneracies. Here, $k$-NN machine learning regressors trained on single-band magnitudes (*Gaia* $G$ or $V$) eliminate track-folding ambiguity, yielding superior mass recovery ($R^2 \ge 0.96$).
...
All observational datasets, theoretical model grids, synthetic control tables, and derived cluster parameter catalogs underlying this article are available in the \textsc{SPECTRA} repository at \url{https://github.com/matosjp/spectra/tree/main/paper_data}.
